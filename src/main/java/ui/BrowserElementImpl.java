...
public class BrowserElementImpl implements BrowserElement {
    ...
    public List<WebElement> findElements(By by) {
        return webElement.findElements(by).stream().map(e -> new BrowserElementImpl(e,driver,methodInvokedOn,this.loc)).collect(Collectors.toList());
    }
    ...
    private void webElementlogger()  {
        logger.info("ThreadID: "+Thread.currentThread().getId()+"  "+Thread.currentThread().getName()+" "+methodInvokedOn);
        ...
        if(false) {
            ...
            try {
				ExtentTestManager.getTest().info(ScreenShotEditor.markUp(ScreenShotEditor.newInstance().editScreenShot(new File(screenShot), this.methodInvokedOn, "INTERMIDIATE STEP"), fileName));
			} catch (Exception e1) {
    ...
    public BrowserElement findById(String id) {
        /*try {
            logger.info("ThreadID: "+Thread.currentThread().getId()+"  "+Thread.currentThread().getName()+" "+("Element Loc "+this.loc.getCanonicalPath());
        }
        catch (Exception e){
            e.printStackTrace();
        }*/
    ...
    public BrowserElement findByLinkText(String linkText) {
        return new BrowserElementImpl(webElement.findElement(By.linkText(linkText)), driver, this.methodInvokedOn,this.loc);
    }

    @Override
    public BrowserElement findByPartialLinkText(String partialLinkText) {
        return new BrowserElementImpl(webElement.findElement(By.partialLinkText(partialLinkText)), driver, this.methodInvokedOn,this.loc);
    }

    @Override
    public BrowserElement findByClass(String className) {
        return new BrowserElementImpl(webElement.findElement(By.className(className)), driver, this.methodInvokedOn,this.loc);
    }

    @Override
    public BrowserElement findByCssSelector(String cssSelector) {
        return new BrowserElementImpl(webElement.findElement(By.cssSelector(cssSelector)), driver, this.methodInvokedOn,this.loc);
    }

    @Override
    public BrowserElement findByTagName(String tagName) {
        return new BrowserElementImpl(webElement.findElement(By.tagName(tagName)), driver, this.methodInvokedOn,this.loc);
    }
