...
public class TestCase19732 extends BaseTest {
	...
	@Test(alwaysRun = true, dataProviderClass = DynamicDataProvider.class, dataProvider = "createAdminUser",
			description = "Training_CD completions_E-signatures for completions",  groups = {"cfr_f","cfr_f.training"
			})
	public void Training_CD_completions_E_signatures_for_completions(String userName) throws Exception {
		...
		usersPage.openOptionsPage().openEsignatureRequirements().openEditRequirements()
				.uncheckReqEsigCtrlDoc(userName, getData("GENERIC.PASSWORD"), getData("TC19721.SIGNATUREREASON"),
						"RecScr")
		...
		Assert(esignatureRequirementsPage.getEsignatureRequirements(0).contains("images/Checked.jpg"),
				"Precondition 2: e-signatures enabled for custom exam completions");
		optionsPage.openTrainingPage().searchTraining(getData("TC19732.TRAINING"));
		Assert(true, "Precondition 3: Control Document");
		...
		openAddAssignmentDefinition().addAssignment(getData("TC19732.TRAINING"),getFromStorage("randomTag1") + "usr1").returnToAssignmentPage();	
		Assert(true, "Precondition 4: Training has been assigned");
		...
		Assert(homePage.openHistoryPage().openTrainingItemGeneralInfo(getData("TC19732.TRAINING")).getCompletionDate().contains(Tools.getDateTime1())
				...
				historyPage.getCompletionDateTimeZone().contains(Tools.getSystemTimeZone()) && historyPage.getSignatureTimeZone().contains(Tools.getSystemTimeZone()),
		...
		homePage.logOut().signIn(userName, getData("GENERIC.PASSWORD"), getData("GENERIC.AUTOMATION.COMPANYCODE")).openToolsMenu().
